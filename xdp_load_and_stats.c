...
static void init_CR_pairs(int map_fd, __u32 map_type)
{
	printf("Intialising CR pairs\n");
	printf("No of IOT devices: %d,",no_of_IOT);
	printf("No of IOT devices: %d\n",CRP_PER_IOT);
	...
	printf("Sucessful INIT of CR DB\n");	
}
...
int main(int argc, char **argv)
{
	...
	stats_map_fd = find_map_fd(bpf_obj, "cr_db_map");
	if (stats_map_fd < 0) {
